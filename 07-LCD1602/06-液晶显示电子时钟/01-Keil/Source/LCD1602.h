...
void LCDWritedata(unsigned char dat);//LCD1602写数据
void LCDWriteCmd(unsigned char cmd); //LCD1602写命令
void LCDWriteCGRAM(unsigned char *str);  //第一种方案：显示自定义字符
void LEDShowStr(unsigned char x,unsigned char y,unsigned char *Str); //显示字符串
void LCDSetChar(unsigned x,unsigned char y,unsigned char pos,unsigned char *str); //第二种方案：显示自定义字符
void LCDSetPosition(unsigned char x,unsigned char y);//LCD1602定坐标 用于字符显示
unsigned char IntegerToStr(long dat,unsigned char *longstr); //整型转字符串
